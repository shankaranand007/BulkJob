...
***********************************************************************************************/
	...
	sphinxRTServer = function (sphinxip,sphinxkey){		
		...
		if(SPHINXRTCONN[sphinxkey]) 
			SPHINXRTCONN[sphinxkey].destroy();
		
		//- Create the connection variable
		SPHINXRTCONN[sphinxkey] = mysql.createConnection({
			host     : sphinxip, 
		...
		SPHINXRTCONN[sphinxkey].connect(function(err){
			if(!err) {
				console.log(sphinxkey + " - Sphinx Cluster is connected ...");   
			} else {
	...
	SphinxLATUPFun = function (sphinxip,sphinxkey){		
		...
		if(SPHINXLTUPCONN[sphinxkey]) 
			SPHINXLTUPCONN[sphinxkey].destroy();
		
		//- Create the connection variable
		SPHINXLTUPCONN[sphinxkey] = mysql.createConnection({
			host     : sphinxip, 
			port	 : SPHINXONLINDEXPORT['SPHINX_PORT'],
			user     : '',
			password : ''
		...
		//- Establish a new connection
		SPHINXLTUPCONN[sphinxkey].connect(function(err){
			if(!err) {
				console.log(sphinxkey + " - Sphinx Cluster is connected ...");   
			} else {
	...
	SphinxRTUpdate = function (sphinxip, sphinxkey){	
		...
		if(SPHINXRTINUPCONN[sphinxkey]) 
			SPHINXRTINUPCONN[sphinxkey].destroy();
		
		//- Create the connection variable
		SPHINXRTINUPCONN[sphinxkey] = mysql.createConnection({
			host     : sphinxip, 
			port	 : SPHINXONLINDEXPORT['SPHINX_PORT'],
			user     : '',
			password : ''
		...
		//- Establish a new connection		
		SPHINXRTINUPCONN[sphinxkey].connect(function(err){
			if(!err) {
				console.log(sphinxkey + " - Sphinx Cluster is connected ..."); 
			} else {
	...
	SphinxONLUpdate = function (sphinxip, sphinxkey){	
		...
		if(SPHINXONLINUPCONN[sphinxkey]) 
			SPHINXONLINUPCONN[sphinxkey].destroy();
		
		//- Create the connection variable
		SPHINXONLINUPCONN[sphinxkey] = mysql.createConnection({
			host     : sphinxip, 
			port	 : SPHINXONLINDEXPORT['SPHINX_PORT'],
			user     : '',
			password : ''
		...
		//- Establish a new connection		
		SPHINXONLINUPCONN[sphinxkey].connect(function(err){
			if(!err) {
				console.log(sphinxkey + " - Sphinx Cluster is connected ..."); 
			} else {
	...
	exports.bmDbSelect = function(objType,hostName,indexName,selectFields,whereClause,whereValueArr,maxMatchQry,qrycomt,cntflag,next){
		try{
			...
			var SPHINXQLCONN = {};
			if(objType ==1){
				SPHINXQLCONN = global.SPHINXRTCONN[hostName];
			} else if(objType ==2){
				SPHINXQLCONN = global.SPHINXLTUPCONN[hostName];
			} 
			...
			if(whereClause != '' && selectFields != '' && indexName != ''){
				bmSphinxRtDb.mysqli_query(SPHINXQLCONN,Query,sphinxql_execQuery,whereValueArr,"SELECT-RT",function(err,selData){
					var end = bmgeneric.UnixTimeStamp();
					//console.error("BM SphinxRT Select Query Time :" + (end-start));					
					if(!err){
						if(cntflag==2){
							var resultdata = {};
							resultdata['RESULTS'] = selData[0];
							resultdata['TOTAL_FOUND'] = selData[1][0]['total_found'];
							next(err, resultdata);
						} else {
							next(err, selData);
						}
			...
			viewprofileOutput['PARAMETER'] = SelQuery;
			next(viewprofileOutput,{});
		}
	}
	...
	exports.mysqli_query = function(DBCONNECT,query,execQuery,bindDataValue,ErrTxt,resolve){
		...
		if(typeof(DBCONNECT) == "object"){
			DBCONNECT.ping(function (err) {
				if (err) {
					console.error("SPHINXRT_UNABLE_TO_PING_ERR_IN_"+ErrTxt+"_QRY:",err);
					var viewprofileOutput = {};							
					viewprofileOutput['responsecode'] = 2;
					viewprofileOutput['errcode'] = 1;
					viewprofileOutput['Error'] = "SPHINXRT_UNABLE_TO_PING_ERR_IN_"+ErrTxt+"_QRY";
					viewprofileOutput['PARAMETER'] = query;
					resolve(err, viewprofileOutput);
					writeTxtFile += "; \nExecQry : "+bindDataValue;
					writeTxtFile += "; \nQry : "+query;
					writeTxtFile += "; \nDb Host : "+DBCONNECT.config.host;
					writeTxtFile += "; \nDb SPHINXRT_UNABLE_TO_PING_ERR_IN_"+ErrTxt+"_QRY :"+err;
					writeTxtFile += "\n=====================================================";
					bmSphinxRtDb.bmSphinxLogError(ErrTxt+"Qry-RT-ERR",writeTxtFile,1,function(err,rtlogger){});
				}
				DBCONNECT.query(query,bindDataValue, function(err, rows) {
					if (err) {
